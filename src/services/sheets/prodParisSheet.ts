...
import {
  YELLOW_FILL, BRIGHT_YELLOW_FILL, SECTION1_FILL, SECTION_FILL, PROD_PARIS_FILL,
...
export function buildProdParisSheet(ws: Worksheet, form: FormData) {
  ...
  // ROW 32: Info atelier (teal fill)
  ws.getCell('A32').value = 'Information atelier = 1h15 = arrondi à 1h30';
  ws.getCell('A32').font = FONT_DATA;
  for (const c of ['A', 'B', 'C', 'D', 'E']) ws.getCell(`${c}32`).fill = PROD_PARIS_FILL;
  ...
  if (form.margesProdParis.prixVenteAnnonce) {
    ws.getCell('F47').value = form.margesProdParis.prixVenteAnnonce;
